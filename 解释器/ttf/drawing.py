...
#控制画图类型的变量，1：曲线，2：直线，3：矩形，4：文本，5：橡皮,6椭圆类
...
def onLeftButtonMove(event):
    ...
    if what.get()==1:
        ...
        canvas.create_rectangle(event.x-10, event.y-10, event.x+10, event.y+10,outline=backColor, fill=backColor)
...
def onLeftButtonUp(event):
    if what.get()==2:
    	...
    	canvas.create_oval(X.get(), Y.get(), event.x, event.y, fill="")
    yesno.set(0)
    global lastDraw
    lastDraw = 0
...
def Open():
    ...
    if filename:
        ...
        canvas.create_image(1080, 1080, image=image)
menu.add_command(label='Open', command=Open)
#保存
def saveAs():
        f = tkinter.filedialog.asksaveasfile(mode='w', defaultextension='.png')
        print(type(canvas.get()))
        t = canvas.get()
        try:
            f.write(t.rstrip())
        except:
            tkinter.messagebox.showerror(title="天哪!", message="Unable to save file...")
...
def drawoval():
    what.set(6)
menuType.add_command(label='oval', command=drawoval)
