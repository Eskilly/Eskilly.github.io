...
class couldmusic(object):
    ...
    def get_song_info(self, song_id):     
        ...
        song_detail_api = "http://music.163.com/api/song/detail/?id=song_id&ids=[song_id]"      # 歌曲信息API
    ...
    def layout(self):
        ...
        self.root.title("网易云音乐下载器")                                          # 标题
        self.root.geometry("460x300+600+300")                                   # 位置
        self.root.bind("Click", lambda x:self.run())                         # 绑定回车键

        # 歌单ID
        Label(self.root, text = "歌单ID:").grid(row = 0, column = 0)
        Entry(self.root, textvariable = self.playlist_id).grid(row = 0, column =1, sticky=N+W)
        
        # 歌曲ID
        Label(self.root, text = "歌曲ID:").grid(row = 1, column =0)
        Entry(self.root, textvariable = self.song_id).grid(row = 1,column = 1, sticky=N+W)

        # 正在下载的歌曲信息
        Label(self.root, text = "正在下载:").grid(row = 2, column =0)
        Label(self.root, textvariable = self.song_name).grid(row = 2,column = 1, sticky=N+W)

        # 进度条
        self.canvas = Canvas(self.root,height=26,width=20, bg="white")
        self.canvas.grid(row = 3,  column = 1,sticky=N+E+S+W)
